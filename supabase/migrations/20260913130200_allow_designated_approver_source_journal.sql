...
-- Permit the designated approver to post only the journal generated for the
-- SUBMITTED payment/expense currently being approved. This is not a general
-- journal-posting permission and does not grant table writes.
...
declare
 ...
 v_source_table text; v_source_id uuid; v_source_status text; v_designated_source boolean:=false;
begin
 if auth.uid() is null then raise exception 'Authentication required'; end if;
 v_role:=public.current_finance_role();
 select status::text,transaction_date,source_table,source_id
   into v_status,v_date,v_source_table,v_source_id
 ...
 if public.is_dr_amany_payment_expense_approver() and v_source_table in ('student_payments','expenses') and v_source_id is not null then
   execute format('select status::text from public.%I where id=$1',v_source_table) into v_source_status using v_source_id;
   v_designated_source:=v_source_status='SUBMITTED';
 end if;
 if v_role not in ('OWNER','CEO','FINANCE_MANAGER') and not v_designated_source then
   raise exception 'Only Finance Admin can post journals';
 end if;
